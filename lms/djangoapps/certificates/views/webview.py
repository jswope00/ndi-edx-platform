...
def _update_course_context(request, context, course, course_key, platform_name):
    ...
    # course_start_date = course.start.date().strftime("%B %d,%y")
    course_start_date = _('{month} {day}, {year}').format(
        month=strftime_localized(course.start, "%B"),
        day=course.start.day,
        year=course.start.year
    )
    #  course_end_date = course.end.date().strftime("%B %d,%y")
    #course_end_date = _('{month} {day}, {year}').format(
    #    month=strftime_localized(course.end, "%B"),
    #    day=course.end.day,
    ...
    try:
        hours_of_effort = context['hours_of_effort']
    except:
        hours_of_effort = "Undefined"
    ...
    #if context['organization_long_name'] or not context['organization_long_name']:
    #if context['organization_long_name']:
    
    if hours_of_effort == "Undefined" and course_end_date == "Undefined": 
        ...
        # Translators:  This text represents the description of course
        logging.info('Hours of EFFORT====:%s' % hours_of_effort)
        if course_end_date != "Undefined" and hours_of_effort != "Undefined":
            context['accomplishment_copy_course_description'] = _('An online course offered by '
                                                                  '{partner_short_name} from {start_date} to {end_date}'
                                                                  ' and with a total of {hours_of_effort} hours of effort.').format(
                partner_short_name=context['organization_short_name'],
                platform_name=platform_name, start_date=course_start_date, end_date=course_end_date,
                hours_of_effort=hours_of_effort)
        elif course_end_date == "Undefined" and hours_of_effort != "Undefined":
            context['accomplishment_copy_course_description'] = _('An online course offered by '
                                                                  '{partner_short_name} from {start_date}'
                                                                  ' with a total of {hours_of_effort} hours of effort.').format(
                partner_short_name=context['organization_short_name'],
                platform_name=platform_name, start_date=course_start_date, end_date=course_end_date,
                hours_of_effort=hours_of_effort)
        elif hours_of_effort == "Undefined" and course_end_date != "Undefined":
            context['accomplishment_copy_course_description'] = _('An online course offered by '
                                                                  '{partner_short_name} from {start_date} to {end_date}.').format(
                partner_short_name=context['organization_short_name'],
                platform_name=platform_name, start_date=course_start_date, end_date=course_end_date,
                hours_of_effort=hours_of_effort)
        else:
            context['accomplishment_copy_course_description'] = _('An online course offered by '
                                                                  '{partner_short_name} from {start_date}.').format(
                platform_name=platform_name, start_date=course_start_date, end_date=course_end_date,
		partner_short_name=context['organization_short_name'],
                hours_of_effort=hours_of_effort)
...
def _track_certificate_events(request, context, course, user, user_certificate):
    ...
    if request.user and request.user.id != user.id:
        emit_certificate_event('evidence_visited', user, course.id, course, {
    ...
    if course_run_fields:
        ...
        if course_run_data.get('weeks_to_complete') and course_run_data.get('max_effort'):
            try:
                ...
                logging.info('MAX EFFORT====:%s' % max_effort)
                catalog_data['hours_of_effort'] = weeks_to_complete * max_effort
                logging.info('CALCULATED Hours of EFFORT====:%s' % catalog_data['hours_of_effort'])
            except ValueError:
